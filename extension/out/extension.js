...
class CodeGraphProvider {
    ...
    getChildren(element) {
        ...
        switch (element.contextValue) {
            case 'dependencies':
                return Promise.resolve(this.analysisResult.dependencies.map(dep => new CodeGraphItem(`${dep.source} → ${dep.target}`, vscode.TreeItemCollapsibleState.None, 'dependency', {
                    command: 'vscode.open',
                    title: 'Open File',
                    arguments: [vscode.Uri.file(dep.source), { selection: new vscode.Range(dep.line, dep.column, dep.line, dep.column) }]
                })));
            case 'complexity':
                return Promise.resolve(this.analysisResult.complexity.map(complex => new CodeGraphItem(`${complex.function} (${complex.complexity})`, vscode.TreeItemCollapsibleState.None, 'complexity-item', {
                    command: 'vscode.open',
                    title: 'Open Function',
                    arguments: [vscode.Uri.file(complex.file), { selection: new vscode.Range(complex.line, 0, complex.line, 0) }]
                })));
            case 'cycles':
                return Promise.resolve(this.analysisResult.cycles.map(cycle => new CodeGraphItem(cycle.cycle.join(' → '), vscode.TreeItemCollapsibleState.None, 'cycle')));
            case 'metrics':
...
class CodeGraphEngine {
    ...
    async detectCycles(projectPath) {
        return new Promise((resolve, reject) => {
    ...
    async generateReport(projectPath, outputPath) {
        return new Promise((resolve, reject) => {
...
function activate(context) {
    ...
    const analyzeProjectCommand = vscode.commands.registerCommand('codegraph.analyzeProject', async () => {
        ...
        try {
            ...
            vscode.window.showInformationMessage(`Analysis complete: ${result.metrics.totalFiles} files, ${result.metrics.dependencyCount} dependencies, ${result.metrics.circularDependencies} cycles`);
        }
        catch (error) {
    ...
    const generateReportCommand = vscode.commands.registerCommand('codegraph.generateReport', async () => {
        ...
        try {
            ...
            const openReport = await vscode.window.showInformationMessage('Report generated successfully!', 'Open Report');
    ...
    context.subscriptions.push(analyzeProjectCommand, showDependencyGraphCommand, checkComplexityCommand, detectCyclesCommand, validateArchitectureCommand, generateReportCommand);
